...
class ClickHouseTableDefinition:
    ...
    @property
    def delete_insert_columns(self) -> tuple[ClickHouseColumn, ...]:
        business_columns_by_name = {
            column.name: column for column in self.business_columns
        }
        delete_business_columns = tuple(
            business_columns_by_name[column_name]
            for column_name in self.order_by_columns
        )

        return delete_business_columns + self.technical_columns
    ...
    def insert_query(self) -> str:
        return _build_insert_query(self.qualified_name, self.insert_columns)

    @property
    def delete_insert_query(self) -> str:
        return _build_insert_query(self.qualified_name, self.delete_insert_columns)
...
def _build_insert_query(
    ...
    return f"INSERT INTO {qualified_name} ({insert_columns}) VALUES"
